"""
Batch Edit Captions Tab UI 组件 - 批量编辑标签
...
class BatchEditCaptionsUI(UIBase):
    ...
    def set_callbacks(self, o_update_filter_and_gallery: List[gr.components.Component], 
                     load_dataset: LoadDatasetUI, filter_by_tags: FilterByTagsUI, 
        ...
        def apply_edit_tags(search_tags: str, replace_tags: str, prepend: bool):
            ...
            dte_instance.replace_tags(search_tags=search_tags, replace_tags=replace_tags, 
                                      filters=get_filters(), prepend=prepend)
            filter_by_tags.tag_filter_ui.get_filter().tags = dte_instance.get_replaced_tagset(
                filter_by_tags.tag_filter_ui.get_filter().tags, search_tags, replace_tags)
            filter_by_tags.tag_filter_ui_neg.get_filter().tags = dte_instance.get_replaced_tagset(
                filter_by_tags.tag_filter_ui_neg.get_filter().tags, search_tags, replace_tags)
        ...
        def search_and_replace(search_text: str, replace_text: str, target_text: str, use_regex: bool):
            if target_text == 'Only Selected Tags':
                ...
                dte_instance.search_and_replace_selected_tags(search_text=search_text, replace_text=replace_text, 
                                                              selected_tags=selected_tags, filters=get_filters(), use_regex=use_regex)
                filter_by_tags.tag_filter_ui.filter.tags = dte_instance.search_and_replace_tag_set(
                    search_text, replace_text, filter_by_tags.tag_filter_ui.filter.tags, selected_tags, use_regex)
                filter_by_tags.tag_filter_ui_neg.filter.tags = dte_instance.search_and_replace_tag_set(
                    search_text, replace_text, filter_by_tags.tag_filter_ui_neg.filter.tags, selected_tags, use_regex)
            elif target_text == 'Each Tags':
                dte_instance.search_and_replace_selected_tags(search_text=search_text, replace_text=replace_text, 
                                                              selected_tags=None, filters=get_filters(), use_regex=use_regex)
                filter_by_tags.tag_filter_ui.filter.tags = dte_instance.search_and_replace_tag_set(
                    search_text, replace_text, filter_by_tags.tag_filter_ui.filter.tags, None, use_regex)
                filter_by_tags.tag_filter_ui_neg.filter.tags = dte_instance.search_and_replace_tag_set(
                    search_text, replace_text, filter_by_tags.tag_filter_ui_neg.filter.tags, None, use_regex)
            elif target_text == 'Entire Caption':
                dte_instance.search_and_replace_caption(search_text=search_text, replace_text=replace_text, 
                                                        filters=get_filters(), use_regex=use_regex)
                filter_by_tags.tag_filter_ui.filter.tags = dte_instance.search_and_replace_tag_set(
                    search_text, replace_text, filter_by_tags.tag_filter_ui.filter.tags, None, use_regex)
                filter_by_tags.tag_filter_ui_neg.filter.tags = dte_instance.search_and_replace_tag_set(
                    search_text, replace_text, filter_by_tags.tag_filter_ui_neg.filter.tags, None, use_regex)
    ...
    def get_common_tags(self, get_filters: Callable[[], List[dte_module.filters.Filter]], filter_by_tags: FilterByTagsUI):
        if self.show_only_selected_tags:
            tags = ', '.join([t for t in dte_instance.get_common_tags(filters=get_filters()) 
