"""
Edit Caption of Selected Image Tab UI 组件 - 编辑选中图片的标签

适配说明：
- 移除 modules.shared / modules.call_queue 依赖
- 移除 _js 回调依赖
- 使用 dte_settings 替代 shared.opts
- 使用 app_config 获取模型路径
"""
...
class EditCaptionOfSelectedImageUI(UIBase):
    ...
    def create_ui(self, cfg_edit_selected):
        ...
        with gr.Tab(label='Read Caption from Selected Image'):
            ...
            self.token_counter_caption = gr.HTML(value='<span>0/75</span>', elem_id='dte_caption_counter')
            with gr.Row():
        ...
        with gr.Column():
            ...
            self.token_counter_edit_caption = gr.HTML(value='<span>0/75</span>', elem_id='dte_edit_caption_counter')
    ...
    def set_callbacks(self, o_update_filter_and_gallery: List[gr.components.Component], 
                     dataset_gallery: DatasetGalleryUI, load_dataset: LoadDatasetUI, 
        ...
        def gallery_index_changed(next_idx: int, prev_idx: int, edit_caption: str, 
                                  copy_automatically: bool, warn_change_not_saved: bool):
            ...
            return [prev_idx if warn_change_not_saved and edit_caption != prev_tags_txt and not self.change_is_saved else -1] + \
        ...
        dataset_gallery.nb_hidden_image_index.change(
            ...
            inputs=[dataset_gallery.nb_hidden_image_index, dataset_gallery.nb_hidden_image_index_prev, 
        ...
        self.btn_hidden_save_caption.click(
            ...
            inputs=[self.tb_hidden_edit_caption, self.nb_hidden_image_index_save_or_not, 
                   self.cb_sort_caption_on_save, self.rb_sort_by, self.rb_sort_order],
        ...
        def interrogate_selected_image(interrogator_name: str, use_threshold_booru: bool, 
                                       threshold_booru: float, use_threshold_waifu: bool, 
                                       threshold_waifu: float):
            if not interrogator_name:
                return ''
            threshold_booru = threshold_booru if use_threshold_booru else dte_settings.interrogate_deepbooru_score_threshold
            threshold_waifu = threshold_waifu if use_threshold_waifu else -1
            return dte_module.interrogate_image(
                dataset_gallery.selected_path, interrogator_name, 
                threshold_booru, threshold_waifu, 
                model_dir=app_config.tagger_model_dir
            )

        self.btn_interrogate_si.click(
            fn=interrogate_selected_image,
            inputs=[self.dd_intterogator_names_si, load_dataset.cb_use_custom_threshold_booru, 
                   load_dataset.sl_custom_threshold_booru, 
                   load_dataset.cb_use_custom_threshold_waifu, load_dataset.sl_custom_threshold_waifu],
