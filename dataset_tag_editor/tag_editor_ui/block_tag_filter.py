...
class TagFilterUI:
    ...
    def create_ui(self, get_filters: Callable[[], List[filters.Filter]], 
                  logic=TagFilter.Logic.AND, sort_by=SortBy.ALPHA, 
                  sort_order=SortOrder.ASC, prefix=False, suffix=False, regex=False):
    ...
    def cbg_tags_changed(self, cbg_tags):
    ...
    def cbg_tags_update(self):
        ...
        if self.filter_mode == TagFilter.Mode.INCLUSIVE:
            tags = dte_instance.get_filtered_tags(self.get_filters(), self.filter_word, 
                                                  self.filter.logic == TagFilter.Logic.AND, 
                                                  prefix=self.prefix, suffix=self.suffix, regex=self.regex)
        else:
            tags = dte_instance.get_filtered_tags(self.get_filters(), self.filter_word, 
