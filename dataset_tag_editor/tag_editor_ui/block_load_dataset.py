"""
Load Dataset UI 组件 - 数据集加载

适配说明：
- 移除 modules.shared / modules.shared.opts 依赖
- 使用 dte_settings 替代 shared.opts
- 使用 config.app_config 获取模型路径
"""
...
class LoadDatasetUI(UIBase):
    ...
    def set_callbacks(self, o_update_filter_and_gallery: List[gr.components.Component], 
                     toprow: ToprowUI, dataset_gallery: DatasetGalleryUI, 
                     filter_by_tags: FilterByTagsUI, filter_by_selection: FilterBySelectionUI, 
        ...
        def load_files_from_dir(
            ...
            threshold_booru = custom_threshold_booru if use_custom_threshold_booru else dte_settings.interrogate_deepbooru_score_threshold
            ...
            dte_instance.load_dataset(
                dir, caption_file_ext, recursive, load_caption_from_filename, 
                replace_new_line, interrogate_method, use_interrogator_names, 
                threshold_booru, threshold_waifu, 
                dte_settings.dataset_editor_use_temp_files, 
                kohya_json_path if use_kohya_metadata else None, 
                dte_settings.dataset_editor_max_res,
                model_dir=model_dir
            )
        ...
        self.btn_load_datasets.click(
            ...
            inputs=[self.tb_img_directory, self.tb_caption_file_ext, 
                   self.cb_load_recursive, self.cb_load_caption_from_filename, 
                   self.cb_replace_new_line_with_comma, self.rb_use_interrogator, 
                   self.dd_intterogator_names, 
                   self.cb_use_custom_threshold_booru, self.sl_custom_threshold_booru, 
                   self.cb_use_custom_threshold_waifu, self.sl_custom_threshold_waifu, 
                   toprow.cb_save_kohya_metadata, toprow.tb_metadata_output],
            outputs=[dataset_gallery.gl_dataset_images, filter_by_selection.gl_filter_images] +
